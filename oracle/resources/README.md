...
- `RSLHelper.db` (~1.4 MB) — a real RslHelper.db (named `<accountId>_RSLHelper.db` on disk; the
  generator globs `*RSLHelper.db`). **Contains personal account data** (roster, gear, records), so it
  stays local — and don't commit the real filename either, since the account id is in it. Sources:
